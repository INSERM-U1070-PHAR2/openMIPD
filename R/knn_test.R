...
#'
#' @param final_fit The trained KNN model from the knn_train function.
#' @param test A preprocessed (ml_data_preprocess function) training dataset with one line per patient.
#' @param test Seed.
#' @returns A test data with containing the KNN predictions.
#' @export
#' @import tidymodels
#' @import vip
#' @import shapviz
...
#' @import stacks
#' @examples
#' results <- knn_test(final_fit = final_fit, test = AMOX_CMIN_TEST)

knn_test <- function(final_fit,
                test,
                seed = 1991) {
