...
#'
#' @param final_fit The trained RF model from the rf_train function.
#' @param test A preprocessed (ml_data_preprocess function) training dataset with one line per patient.
#' @param seed Seed for reproducibility.
#' @returns A test data with containing the RF predictions.
#' @export
#' @import tidymodels
#' @import vip
#' @import shapviz
...
#' @import ranger
#' @examples
#' results <- rf_test(final_fit = final_fit, test = AMOX_CMIN_TEST)

rf_test <- function(final_fit,
                test,
                seed = 1991) {
